...
function RRRow({
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl bg-[#071828]/60 border border-white/[0.08] p-5 md:p-6"
    >
      <div className="flex items-center justify-between mb-4">
        <span className="text-sm font-semibold text-white/70">{label}</span>
        <span className="text-sm font-bold" style={{ color: tone === "jade" ? "#33eab8" : "rgba(255,255,255,0.4)" }}>
          {reward}
        </span>
      </div>
      <div className="space-y-2.5">
        <div className="flex items-center gap-3">
          <span className="w-14 text-[11px] uppercase tracking-wide text-white/35">Risk</span>
        ...
        <div className="flex items-center gap-3">
          <span className="w-14 text-[11px] uppercase tracking-wide text-white/35">Reward</span>
          <div className="flex-1 h-3 rounded-full bg-white/[0.05] overflow-hidden">
      ...
      <section className="relative w-full bg-[#040e1c] py-24 px-4 md:px-6 overflow-hidden">
        <div className="relative z-10 max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.7 }} className="text-center mb-14">
            <p className="text-xs md:text-sm uppercase tracking-[0.3em] text-[#33eab8] mb-4">Why most traders bleed</p>
