...
import { Check, ShieldCheck, Gift, Zap, Target, Clock, Layers } from "lucide-react";
  ...
  return (
    <MotionConfig reducedMotion="user">
      <section ref={sectionRef} className="relative w-full bg-[#040e1c] py-[5.5rem] px-4 md:px-6 overflow-hidden">
        {/* Decorative trading-desk background, text sits on top so it stays subtle */}
        <motion.div
        ...
        >
          <motion.img
            ...
            alt="Trading desk in low light"
            style={{ y, width: "100%", height: "120%", objectFit: "cover", opacity: 0.2, display: "block", willChange: "transform" }}
          />
        </motion.div>
        <div className="absolute inset-0 bg-gradient-to-b from-[#040e1c] via-[#040e1c]/65 to-[#040e1c] pointer-events-none -z-10" />
        ...
        <div className="relative z-10 max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.7 }} className="text-center mb-14">
            <Eyebrow icon={Layers}>The complete system</Eyebrow>
          ...
          <div className="grid gap-4 md:grid-cols-3 mb-14">
            {levers.map((l, i) => (
              <motion.div key={l.title} initial={{ opacity: 0, y: 26 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.55, delay: i * 0.1 }} className="rounded-2xl bg-[#071828]/60 border border-white/[0.08] p-6">
                ...
                <p className="text-base text-white/50 leading-snug">{l.body}</p>
              </motion.div>
            ))}
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.7 }} className="rounded-3xl border border-[#00e5a8]/25 bg-[#071828]/70 p-7 md:p-10 shadow-[0_0_60px_-20px_rgba(0,229,168,0.35)]">
              ...
              {stack.map((s) => (
                <li key={s.name} className="flex items-center justify-between gap-4 py-3.5">
                  <span className="flex items-start gap-3">
                    ...
                    <span className={"text-base md:text-lg " + (s.bonus ? "text-[#f5a623]/90 font-medium" : "text-white/80")}>{s.name}</span>
                  </span>
                  <span className="text-base md:text-lg text-white/35 line-through tabular-nums shrink-0">{s.value}</span>
                </li>
              ))}
            </ul>
            ...
            <div className="mt-7 pt-6 border-t border-white/[0.10] flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-center sm:text-left">
                <p className="text-base text-white/45">Total real value</p>
                <p className="text-2xl md:text-3xl font-black text-white/40 line-through tabular-nums">$9,800</p>
              </div>
              <div className="text-center sm:text-right">
                <p className="text-base text-[#33eab8] uppercase tracking-wide">Yours today</p>
                <p className="text-4xl md:text-5xl font-black text-white tabular-nums">$5,999</p>
              </div>
            </div>

            <PrimaryCTA size="md" fullWidth className="mt-7">Lock in The Method for $5,999</PrimaryCTA>
          </motion.div>
          ...
          <p className="text-center text-sm text-white/25 mt-5 max-w-xl mx-auto">
